			   ...
			   struct bio *bio)
{
	...
	/* BUG - Workaround until cause of a null pointer dereference
	 * due to empty elv.icq is found - Andrei */
	struct cfq_io_cq *rq_cic;
	rq_cic = RQ_CIC(rq);
	
	if (rq_cic != NULL) {

		if (current->io_context == rq_cic->icq.ioc) {
			cic = rq_cic;
		} else {
