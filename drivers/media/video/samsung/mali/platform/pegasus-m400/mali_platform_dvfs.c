...
static void mali_platform_wating(void)
{
...
static mali_bool mali_dvfs_table_update(void)
{
	...
	unsigned int i;
	exynos_result_of_asv = mali_asv_group;
...
{
	unsigned int i;
	mali_asv_group = asv_group;

	for (i = 0; i < MALI_DVFS_STEPS; i++) {
		mali_dvfs[i].vol = asv_3d_volt_9_table[i][asv_group];
		MALI_PRINT(("mali_dvfs[%d].vol = %d\n", i, mali_dvfs[i].vol));
	}
}
#endif
