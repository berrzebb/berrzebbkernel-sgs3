...
/* read LCD : to check LCD-alive */
static ssize_t medic_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct lcd_info *lcd = dev_get_drvdata(dev);
	char buffer[16];
	...
	ret = s6e8ax0_read(lcd, LDI_ID_REG, LDI_ID_LEN, buffer, 3);
	if (!ret) {
		sprintf(temp, "read fail\n" );
	} else	{
		sprintf(temp, "Read ID : 0x%02X, 0x%02X, 0x%02X\n", buffer[0], buffer[1], buffer[2]  );
	}
...
/* import function from midas-lcd.c */
int reset_lcd(void);
void lcd_cfg_gpio(void);
int lcd_power_on(void *ld, int enable);

/* lcd off -> on sequence */
static ssize_t medic_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	struct lcd_info *lcd = dev_get_drvdata(dev);
	int value;
	...
	if (rc < 0)
		...
		dev_err(&lcd->ld->dev, "%s : LCD off/on sequence.\n", __func__ );

		/* off sequence */
		s6e8ax0_power_off( lcd );
		lcd_power_on( (void*)0xFF, 0 ); // 0xFF = dummy */

		msleep(500);

		/* on sequence */
		lcd_power_on( (void*)0xFF, 1 ); // 0xFF = dummy */
		msleep(500);
		reset_lcd();
		msleep(500);
		s6e8ax0_power_on( lcd );
	}
	return size;
}

static DEVICE_ATTR(medic, 0664, medic_show, medic_store);
...
static int s6e8ax0_probe(struct device *dev)
{
	...
	ret = device_create_file(&lcd->ld->dev, &dev_attr_medic );
	if (ret < 0)
		dev_err(&lcd->ld->dev, "failed to add sysfs entries, %d\n", __LINE__);
